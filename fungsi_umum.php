<?php
  ...
  // Cara pakai : jamTanggalIndonesia(#tanggal dan waktu);
  ...
  // Sensor sebagian string
  // Cara pakai : echo sensorString($target_string, $angka_mulai, $angka_jumlah_sensor);
  function sensorString($string, $start, $length) {
    // Mengambil sebagian string
    $maskedString = substr($string, $start, $length);
    // Mengganti karakter yang diambil dengan karakter '*'
    $maskedString = str_repeat('*', strlen($maskedString)) . substr($maskedString, -strlen($string) + $start + $length);
    return $maskedString;
  }

  // Format Angka K
  // Cara Pakai : echo formatAngkaK(#angka);
